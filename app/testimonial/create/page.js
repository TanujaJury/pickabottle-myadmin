...
export default function ManageTestimonialPage() {
    const searchParams = useSearchParams();
    const editId = searchParams.get("edit"); // ?edit=3
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                setForm({
                    name: item.name,
                    message: item.description,  // IMPORTANT FIX
                    profile_url: null,
    ...
    const handleChange = (e) => {
        ...
        if (name === "profile_url") {
            ...
            if (file) {
                ...
                setPreview(URL.createObjectURL(file)); // correct preview
            }
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.profile_url) {
            fd.append("profile_url", form.profile_url);
        }
        ...
        try {
            if (isEdit) {
                ...
                setForm({ name: "", message: "", profile_url: null, old_image: "" });
                setPreview("");
            }
